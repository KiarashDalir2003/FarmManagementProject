    ...
    , ui(new Ui::LoginPage)
{
    ...
    password_regex.setPattern("^(?=.*[0-9])(?=.*[a-zA-Z])[a-zA-Z0-9]{8,}$");
    phoneNum_regex.setPattern("\\d+");
    money_regex.setPattern("\\d+");
    ...
    StartSound->setMedia(
                QUrl::fromLocalFile("C:\\Users\\Microsoft\\Desktop\\FarmManegementProject\\sounds\\StartSound.mp3"));
    StartSound->play();
    ...
    mydatabase.setDatabaseName("C:\\Users\\Microsoft\\Desktop\\FarmManegementProject\\ProjectDatabase.db");
    mydatabase.open();
    if (!mydatabase.open())
    {
        NagativeSound->play();
...
void LoginPage::on_ContinueBtn_clicked()
{
     ...
     {
         NagativeSound->play();
         QMessageBox::warning(this, "Error", "This account has the same username , email or phone number.");
